...
public class LoginClass {
    ...
    public static void addLoggerToClass(CtClass<?> ctClass) {
        ...
        CtField<?> loggerField = ctClass.getFactory().Field()
                .create(
                        ctClass,
                        Set.of(ModifierKind.PRIVATE, ModifierKind.STATIC, ModifierKind.FINAL),
                        ctClass.getFactory().Type().createReference(Logger.class),
                        "logger",
                        ctClass.getFactory().Code().createCodeSnippetExpression("LoggerFactory.getLogger(" + ctClass.getSimpleName() + ".class)")
                );
